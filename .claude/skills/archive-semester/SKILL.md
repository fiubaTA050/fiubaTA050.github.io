...
**Do not touch the "Cuatrimestres Anteriores" `<select>` in `_includes/footer.html`.** Linking the live site to a new archive is a separate, manual decision. This skill only produces the snapshot.

**1. Build into a temp destination, with the archive baseurl:**
...
**2. Copy the snapshot into place:**
...
**3. Verify** — run `bundle exec jekyll serve` and crawl `/archive/2026a/` checking every link and asset for a 200, including the footer selector's link to sibling archives. Confirm pages render styled (`assets/styles.css` + `assets/theme.js` resolve under the archive prefix), not just that they return 200.
...
- **A running `jekyll serve` will corrupt the snapshot.** Its watcher rebuilds `_site` (without `--baseurl`) whenever a source file changes, racing the copy. Building into `$TMP` instead of `_site` sidesteps this entirely — that's why step 1 uses `--destination`. It also leaves the dev server serving the live site the whole time.
